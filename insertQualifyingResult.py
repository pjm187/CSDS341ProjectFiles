...
TABLE_DRIVER    = "driver"
TABLE_TEAM      = "teamseason"
TABLE_RACE      = "race"
TABLE_QUAL      = "qualifyingresult"
...
driver_data  = supabase.table(TABLE_DRIVER).select("driverid, name").execute().data
team_data    = supabase.table(TABLE_TEAM).select("teamid, shortname").execute().data
gp_data      = supabase.table("grandprix").select("gpid, gpname").execute().data
race_data    = supabase.table(TABLE_RACE).select("raceid, seasonyear, gpid").execute().data

gpid_to_name = {
    row['gpid']: str(row['gpname']).strip().lower()
    for row in gp_data
}
...
race_aliases = {
    ...
    "united states":    ["united states", "usa", "americas"],   # 2022 CSV uses full name
    ...
    "france":           ["french", "france", "paul ricard"],   # present in 2022 season
}
...
def get_race_id(season_year, track_name):
    """Resolves a CSV track name + year to a database RaceID."""
    if not track_name:
        return None
...
def get_team_id(team_str):
    """Resolves a CSV team name to a database TeamID."""
    if not team_str:
        return None
...
def get_driver_id(driver_raw):
    """Resolves a CSV driver string to a database DriverID.
    Strips the 3-letter abbreviation suffix (e.g. 'Max Verstappen VER')
    then falls back to fuzzy matching if a direct lookup fails.
    """
    if not driver_raw:
        return None
    clean_name = (
...
def normalize_lap_time(raw):
    """Returns the lap time string as-is if non-null, else None.
    Keeps the original format (e.g. '1:30.558') for VARCHAR storage.
    """
    if pd.isna(raw) or str(raw).strip() in ("", "nan"):
        return None
    return str(raw).strip()

qualifying_directory = './qualifyingresults'
all_qual_entries     = []

for filename in os.listdir(qualifying_directory):
    filename_lower = filename.lower()

    if not (filename_lower.startswith("formula1_") and "qualifying" in filename_lower):
        continue

    filepath = os.path.join(qualifying_directory, filename)
    ...
    for _, row in df.iterrows():
        ...
        if race_id and driver_id and team_id:
            all_qual_entries.append({
                'raceid':   race_id,
                'driverid': driver_id,
                'teamid':   team_id,
                'position': row.get('position'),
                'q1time':   normalize_lap_time(row.get('q1time')),
                'q2time':   normalize_lap_time(row.get('q2time')),
                'q3time':   normalize_lap_time(row.get('q3time')),
            })

if not all_qual_entries:
    print("\nError: No matching records compiled. Check directory name and CSV filenames.")
    exit(1)

qual_df = pd.DataFrame(all_qual_entries)

qual_df = qual_df.drop_duplicates(subset=['raceid', 'driverid'])

qual_df.insert(0, 'qualid', range(1, 1 + len(qual_df)))

qual_df = qual_df.astype(object).where(pd.notna(qual_df), None)

data_to_insert = qual_df.to_dict(orient='records')

int_columns = ['qualid', 'raceid', 'driverid', 'teamid', 'position']
...
print(f"\nUpserting {len(data_to_insert)} rows into {TABLE_QUAL}...")
response = supabase.table(TABLE_QUAL).upsert(data_to_insert).execute()
print("Qualifying Result insertion complete!")